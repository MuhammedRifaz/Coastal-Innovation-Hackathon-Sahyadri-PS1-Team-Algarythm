"""Tests for the fleet assignment and mission reassessment on flood changes."""
...
def test_assignment_flips_after_bridge_flood_vs_naive_nearest(service: GraphService):
    """At (12.84, 74.86), Unit 3 (south bank) is the straight-line-nearest
    vehicle, but it needs the bridge to get there. Before flooding it's
    still reachable; after flooding the bridge it becomes unreachable,
    so the true route-cost assignment must flip to a north-bank unit —
    exactly the "naive nearest goes wrong" case the master plan wants."""
    from app.core import fleet

    lat, lng = 12.84, 74.86
    incident_node = service.snap_to_node(lat, lng)

    naive_before = fleet.nearest_by_straight_line(service.vehicles, (lng, lat), service._node_coords)
    assert naive_before is not None and naive_before.callsign == "Unit 3"

    for edge_id in CRITICAL_BRIDGE_EDGE_IDS:
        service.apply_flood(edge_id, 60.0)

    naive_after = fleet.nearest_by_straight_line(service.vehicles, (lng, lat), service._node_coords)
    assert naive_after is not None and naive_after.callsign == "Unit 3"  # naive logic never learns

    snapshot = service.create_incident(lat=lat, lng=lng, severity=2)
    mission = snapshot.missions[0]
    assigned_vehicle = next(v for v in service.vehicles if v.id == mission.vehicle_id)

    # The real assignment must NOT be the naive choice — Unit 3 can't
    # reach this incident anymore.
    assert assigned_vehicle.callsign != "Unit 3"
    assert mission.route.reachable
    assert any("Unit 3" in reason for reason in mission.reasons)
